...
@Composable
fun CaregiverHomeScreen(navController: NavController) {
    val role = getRoleByUserId()

    if (!LocalInspectionMode.current && FirebaseAuth.getInstance().currentUser == null)  {
        navController.navigate("landing") {
            popUpTo("caregiver home") { inclusive = true }
            launchSingleTop = true
        }
    }

    if(role.equals(UserRole.CAREGIVER)) {
        navController.navigate("employee home") {
            popUpTo("caregiver home") { inclusive = true }
            launchSingleTop = true
        }
    }

    val nameSurname = getNameSurnameByUserId()
